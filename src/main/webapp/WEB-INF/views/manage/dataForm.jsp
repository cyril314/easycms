        ...
        <c:choose>
                ...
                <div class="control-group">
                    <label class="control-label">
                        <c:if test="${'0' == item.allowBlank }"><font color="red">*</font></c:if>
                            ${item.name }:</label>
                    <div class="controls">
                        <input type="text" name="${mappingData }" maxlength="${item.length }" style="width:550px;"
                               <c:if test="${'0' == item.allowBlank }">class="required"</c:if> value="${fns:getProperty(entity, mappingData)}"/>
            ...
            <c:when test="${'number' == item.type}">
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input type="text" name="${mappingData }" maxlength="${item.length }" style="width:550px;"
                               class="number <c:if test="${'0' == item.allowBlank }">required</c:if>" value="${fns:getProperty(entity, mappingData)}"/>
                        <span class="help-inline"> ${item.remark }</span>
                    </div>
                </div>
            </c:when>
                ...
                <div class="control-group">
                    <label class="control-label">
                        <c:if test="${'0' == item.allowBlank }"><font color="red">*</font></c:if>
                            ${item.name }:</label>
                    <div class="controls">
                        <textarea name="${mappingData }" maxlength="${item.length }" class="<c:if test="${'0' == item.allowBlank }">required</c:if>" rows="3"
                                  style="width: 600px;">${fns:getProperty(entity, mappingData)}</textarea>
                        <span class="help-inline"> ${item.remark }</span>
                    </div>
                </div>
            </c:when>
                ...
                <div class="control-group">
                    <label class="control-label">
                        <c:if test="${'0' == item.allowBlank }"><font color="red">*</font></c:if>
                            ${item.name }:</label>
                    <div class="controls">
                        <input type="text" name="${mappingData }" class="Wdate <c:if test="${'0' == item.allowBlank }">required</c:if>" pattern="yyyy-MM-dd"
                               style="width: 120px;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"
                               value="${fns:getProperty(entity, mappingData)}"/>
                        <span class="help-inline"> ${item.remark }</span>
                    </div>
                </div>
            </c:when>
                    ...
                    <div class="controls">
                        <select name="${mappingData }" class="<c:if test="${'0' == item.allowBlank }">required</c:if>">
                            ...
                            <c:forEach items="${item.combo}" var="op">
                                <option value="${op.value}" <c:if
                                        test="${op.value == fns:getProperty(entity, mappingData)}"> selected </c:if> >${op.value}</option>
                            </c:forEach>
                        </select>
                        <span class="help-inline"> ${item.remark }</span>
                    </div>
