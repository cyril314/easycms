...
public class ManageServlet extends BaseServlet {
    ...
    private void toDataList(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        request.setAttribute("colModel", FastJsonUtils.toJSON(colModel));
    ...
    private List<Map<String, Object>> getColModel(List<Column> columns) {
        List<Map<String, Object>> cols = newArrayList();
        Map<String, Object> idKey = newHashMap();
        idKey.put("label", "主键");
        idKey.put("name", "id");
        idKey.put("key", true);
        ...
        for (Column c : columns) {
            Map<String, Object> col = newHashMap();
    ...
    private void loadData(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtil.isNotEmpty(date1)) {
            entity.setDate1(DateUtils.dateParse(date1));
        }
        if (StringUtil.isNotEmpty(date2)) {
            entity.setDate2(DateUtils.addDay(DateUtils.dateParse(date2), 1));
        }
